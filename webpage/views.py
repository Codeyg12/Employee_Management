...
model_map = {
    "employee": {
        ...
        "table_data": ["first_name", "last_name", "employee_role"],
    },
    "role": {
        "name": Role,
        "form": AddRoleForm,
        "headers": ["Title", "Salary", "Department"],
        "table_data": ["title", "salary", "role_department"],
    },
    "department": {
        "name": Department,
        "form": AddDepartmentForm,
        "headers": ["Department Name"],
        "table_data": ["department_name"],
    },
}
...
def role(request, pk):
    if request.user.is_authenticated:
        ...
        delete_url = reverse("delete_record", kwargs={"name": "role", "pk": role.id})
        return render(request, "role.html", {"role": role, "delete_url": delete_url})
    else:
        messages.error(request, loginNeeded)
        return redirect("home")
